...
HWMonitorWidget::HWMonitorWidget(QWidget *parent) : QWidget(parent)
{
	...
	connect(m_pTimer,&QTimer::timeout,this,[this](){ this->update(); });
	m_pTimer->start();
}
...
void HWMonitorWidget::paintEvent(QPaintEvent *event)
{
	...
	*/
	...
	if(y != this->height()) emit signal_heightChangeRequest(y);
}
...
void HWMonitorWidget::slot_mouseReleaseEvent(QMouseEvent *event)
{
	bool find = false;
	...
	if( !find ){
		m_fixed = !m_fixed;
		emit signal_clickObject( ui_skrepka, m_fixed );
	}
